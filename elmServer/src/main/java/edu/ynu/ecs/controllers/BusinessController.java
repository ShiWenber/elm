...
public class BusinessController extends AbstractTypedController<Business, String> {
	...
	@Operation(summary = "商家查看商品列表方法", security = @SecurityRequirement(name = "Authorization"))
	@Parameter(name = "businessId", description = "商家id", required = true)
	@GetMapping("/listItem")
	public List<Item> listItem(@RequestParam(value = "businessId") String businessId) {
		return businessService.listItem(businessId);
	}

	@Operation(summary = "商家分页查看商品列表方法",
			security = @SecurityRequirement(name = "Authorization"))
	@Parameter(name = "businessId", description = "商家id", required = true)
	@GetMapping("/listItem/{businessId}")
	public Page<Item> listItem(@PathVariable(value = "businessId") String businessId, Pageable pageable) {
		List<Item> list = businessService.listItem(businessId);
		return itemService.listToPage(list, pageable);
	}
